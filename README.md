...
| 走路 | 空闲时会在当前位置附近小范围散步（左右踱步+起伏摇摆），拖动/点击即中断；右键菜单可手动「散步」，关键词彩蛋也支持 `action: "walk"` |
| 提醒 | 喝水/活动/休息定时提醒（气泡台词+指定动作），间隔与台词均可在配置中修改，右键菜单可单独开关 |
...
1. 解压 `桌宠绿色版_v2.3.2.zip` 到任意目录（建议非系统盘，如 `D:\桌宠`）；
2. 双击 `DesktopPet.exe`；
3. 桌宠出现在屏幕右下角，开箱即用。
...
| 右键 | 菜单：大小调整 / 置顶开关 / 打开对话面板 / 散步 / 提醒开关 / 更换形象 / 导入·导出人格包 / 打开人格文件夹 / 退出 |
| 拖图片到桌宠上 | 自动抠图并更换形象 |
...
```json
"behavior": {
  "walk_enabled": true,     // 空闲时允许散步
  "walk_range_px": 180,     // 散步范围（±像素）
  "walk_speed_px_s": 70     // 散步速度
},
"reminders": {
  "enabled": true,          // 提醒总开关（右键菜单也可切换）
  "drink": {"enabled": true, "interval_min": 45,      // 喝水提醒
            "lines": ["到喝水时间了，起身接杯水，顺便让眼睛休息一下吧"],
            "action": "jump"},
  "move":  {"enabled": true, "interval_min": 90,      // 活动提醒
            "lines": ["坐得有些久了，站起来活动一下身体吧"],
            "action": "dance"},
  "rest":  {"enabled": true, "interval_min": 150,     // 休息提醒
            "lines": ["连续工作很久了，闭目休息五分钟吧"],
            "action": "squish"}
}
```

提醒触发时弹气泡 + 播放 `action` 指定动作（可留空），间隔 ±10% 随机抖动，避免每次都卡在整点。
...
- `dist/桌宠绿色版_v2.3.2.zip` — 压缩包，发给接收方**解压双击 exe 即用**。
...
**Q8：双击后没反应 / 想同时开两个桌宠？**
程序是单实例的：若桌宠已在运行，再次启动会自动把桌宠窗口叫到面前（而不是再开一个）。
若桌宠"跑丢"到屏幕外找不到了，再双击一次即可唤回。想同时养两只的话，
修改 `main.py` 中 `INSTANCE_NAME` 后重新打包即可。
